...
Version: 1.0 | Status: Implementation-Oriented Specification | Date: March 1, 2026\
...
> **Implementation stack authority:** For current technology choices, use [../ADR-011_STACK_STANDARDIZATION_AND_GHATANA_PLATFORM_ALIGNMENT.md](../ADR-011_STACK_STANDARDIZATION_AND_GHATANA_PLATFORM_ALIGNMENT.md). This specification contains logical models and Nepal-specific requirements; ADR-011 defines the active implementation baseline.
...
This specification strictly follows the Generic Core + Jurisdiction Plugin + Operator Pack separation defined in siddhanta.md. No Nepal-specific rule, threshold, format, workflow, settlement constant, tax rate, lock-in rule, margin percentage, circuit breaker value, or regulatory timeline may be hardcoded into core services. All regulatory, exchange, tax, reporting, instrument, calendar, and eligibility logic must be implemented through versioned plugins governed by the lifecycle, contract, and certification model defined in Sections 14–18.
...
| `preference_attributes` | JSON (nullable) | `{dividend_rate, cumulative, participating, conversion_ratio, redemption_date_bs, callable}` | Only for PREFERENCE_* types; supports SEBON’s irredeemable preference share framework |
...
|:---|:---|:---|:---|:---|:---|
| CGT-LISTED-LONG | Capital gains (listed) | 5%  **[VERIFY]** | Holding period > 365 days | Broker (TDS) | Annual |
| CGT-LISTED-SHORT | Capital gains (listed) | 7.5%  **[VERIFY]** | Holding period ≤ 365 days | Broker (TDS) | Annual |
| CGT-UNLISTED | Capital gains (unlisted) | 10%  **[VERIFY]** | Any holding period | Transferor | Annual |
| CGT-PROMOTER | Capital gains (promoter transfer) | 10%  **[VERIFY]** | Promoter share transfer | Transferor | At transfer |
| TDS-DIVIDEND | TDS on cash dividend | 5%  **[VERIFY]** | Resident individuals | Issuer/RTA | At distribution |
| TDS-INTEREST | TDS on debenture interest | 5–15%  **[VERIFY]** | Varies by instrument type | Issuer | At payment |
...
| **National ID / eKYC interface** | Identity verification adapter | Approval-dependent interface (TBD) | Request-Response | ID verification requests/responses | Per-KYC |
...
```json
{
  ...
  "properties": {
    ...
    "vendor": {
      ...
      "properties": {
        ...
        "sebon_registration": { "type": "string", "description": "SEBON vendor registration number if applicable" }
      },
    ...
    "interfaces": {
      ...
      "properties": {
        "api": {
          "type": "object",
          "properties": {
            "openapi": { "type": "string", "description": "Path to OpenAPI spec" },
      ...
      "properties": {
        ...
        "secrets": {
          ...
          "items": {
            ...
            "properties": {
              "name": { "type": "string" },
              "type": { "type": "string", "enum": ["api_key", "certificate", "oauth_secret", "encryption_key"] }
            }
          }
        },
        "audit": {
      ...
      "properties": {
        "models_used": {
          ...
          "items": {
            ...
            "properties": {
              "name": { "type": "string" },
              "type": { "type": "string", "enum": ["classification", "regression", "nlp", "generative", "recommendation", "anomaly_detection"] },
              "version": { "type": "string" },
              "purpose": { "type": "string" },
              "training_data_description": { "type": "string" },
              "explainability_method": { "type": "string", "enum": ["rule_extraction", "shap", "lime", "attention", "counterfactual", "none"] },
    ...
    "compatibility": {
      ...
      "properties": {
        ...
        "required_ai_services": { "type": "array", "items": { "type": "string" } },
        "required_nepal_integrations": {
          "type": "array",
          "items": {
            "type": "string",
            "enum": ["cdsc_api", "nepse_api", "meroshare_api", "national_id_gateway", "connectips_api", "asba_api", "sebon_portal", "nrb_portal"]
          }
        }
      }
    },
        ...
        "evidence": {
          ...
          "items": {
            ...
            "properties": {
              "type": {
                "type": "string",
                "enum": ["pen_test_report", "ai_model_card", "ai_bias_audit", "integration_conformance", "sebon_approval", "nrb_approval", "performance_test", "security_scan"]
              },
  ...
  "required": ["name", "version", "capabilities", "security", "nepal_compliance", "compatibility"]
}
```
