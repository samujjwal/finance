...
Version: 1.0 | Status: Strategic & Tactical Reference | Date: March 1, 2026
...
>
> **Implementation stack authority:** For current technology choices, use [../ADR-011_STACK_STANDARDIZATION_AND_GHATANA_PLATFORM_ALIGNMENT.md](../ADR-011_STACK_STANDARDIZATION_AND_GHATANA_PLATFORM_ALIGNMENT.md). This strategy document may discuss broader options, but ADR-011 defines the actual implementation baseline.
...
[VERIFY] The national conversation on AI in capital markets has officially begun, with the 'Capital Market in AI Era' interaction reported on January 29, 2026, featuring NEPSE leadership discussing AI's regulatory role. This signals a receptive policy environment. NRB published **AI Guidelines (December 2025)** for banks and financial institutions, providing a governance baseline that Siddhanta can align to, while still requiring conservative rollout of high-impact AI use cases.
...
| **NEPSE** | Stock exchange | Trading rules, listing rules, circuit breakers, market surveillance, member (broker) regulation | [VERIFY] January 29, 2026 AI-in-capital-markets interaction; TMS management; floorsheet publication |
...
- **IPO workflow engine**: manage the entire lifecycle from prospectus / offer-document filing through approval to listing, including post-issue close reporting within the current 7-day Board-notification window (`Ref: LCA-025`, `Ref: LCA-026`).
...
|:---|:---|:---|:---|
| **Capital gains tax (listed securities)** | [VERIFY] 5% (held >1 year) / 7.5% (held <1 year) | On realized gains from secondary market sales | Auto-calculated at trade level; broker deducts TDS; reports for annual filing |
| **Capital gains tax (unlisted/promoter transfer)** | [VERIFY] 10% | On transfer of unlisted shares | Different rate engine for OTC/promoter transfers |
| **TDS on cash dividends** | [VERIFY] 5% | Deducted by issuer/RTA at distribution | Auto-calculated during corporate actions processing |
| **TDS on interest (debentures)** | [VERIFY] 5–15% (varies) | Deducted by issuer | Bond interest payment workflow includes TDS |
...
```json
{
  ...
  "interfaces": {
    ...
    "events": {
      "publishes": ["PromoterLockExpired", "PromoterTransferRequested", "LockViolationDetected"],
      "subscribes": ["TradeExecuted", "SettlementInitiated", "ShareTransferRequested"]
    },
    "data_contracts": [
      {
  ...
  "security": {
    "permissions": ["read.positions", "read.client_pii", "write.lock_status", "export.reg_reports"],
    "secrets": [
  ...
  "certification": {
    ...
    "evidence": [
      { "type": "pen_test_report", "uri": "https://certs.siddhanta.com.np/ptest-2026-q1.pdf" },
      { "type": "ai_model_card", "uri": "https://certs.siddhanta.com.np/lock-detector-card.pdf" },
      { "type": "ai_bias_audit", "uri": "https://certs.siddhanta.com.np/lock-detector-bias.pdf" },
      { "type": "integration_conformance", "uri": "https://certs.siddhanta.com.np/cdsc-conformance.pdf" }
    ]
  }
}
```
...
|:---|:---|:---|:---|
| **Regulatory policy reversal** (e.g., SEBON mandates dual ISINs against our single-source-of-truth model) | Medium | High | **Architectural abstraction:** Core ledger is attribute-based, not ISIN-based. Adapting to dual-ISIN should be a bounded data-model and integration change rather than a full platform rewrite. Work *with* CDSC, not against them. |
...
| **CDSC/NEPSE resistance** (perceived competitive threat to their technology role) | Medium | High | **Complementary positioning:** Position as a value-add layer *on top of* CDSC/NEPSE infrastructure, not a replacement. Offer to white-label or co-develop. Build connectors, not competitors. |
...
| **NEPSE TMS lock-in** (brokers cannot easily change their trading interface) | Medium | Medium | **Complement, don't replace:** Position as back-office automation that works *alongside* NEPSE TMS, not as a replacement trading interface. Integrate via TMS API/feed consumption. |
...
| **Market downturn** (reduced trading volumes compress broker margins and tech budgets) | Medium | Medium | **Demonstrate ROI during downturns:** Platform value proposition (reduced operational cost, fewer errors, compliance automation) is actually *stronger* during downturns when margins are thin. |
...
| [VERIFY] NEPSE “Capital Market in AI Era” interaction (Jan 29, 2026) | Policy-signal claim referencing NEPSE leadership | [VERIFY — add NEPSE official notice/post or authoritative publication] | — | ⛔ Missing | Currently tagged [VERIFY] in the main text. |
...
|---|---|---|---|---|
| Listed securities capital gains | [VERIFY] 5% / 7.5% | Income Tax Act / IRD circular (exact schedule + mechanism) | ⛔ Missing | Replace secondary references with legal text and clause mapping. |
| Unlisted/promoter transfer CGT | [VERIFY] 10% | Income Tax Act schedule | ⛔ Missing | Confirm entity-specific variations if any. |
| Dividend withholding (TDS) | [VERIFY] 5% | Income Tax Act schedule | ⛔ Missing | Confirm resident/non-resident handling. |
| Debenture interest TDS | [VERIFY] 5–15% | Income Tax Act schedule + instrument rules | ⛔ Missing | Must be instrument-type specific. |
