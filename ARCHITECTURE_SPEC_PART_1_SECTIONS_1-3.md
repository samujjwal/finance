...
> **Stack authority**: [ADR-011_STACK_STANDARDIZATION_AND_GHATANA_PLATFORM_ALIGNMENT.md](ADR-011_STACK_STANDARDIZATION_AND_GHATANA_PLATFORM_ALIGNMENT.md) defines the canonical Siddhanta stack. Any older technology labels in this document are illustrative unless they align to ADR-011.
...
**Project Siddhanta** is a **jurisdiction-neutral, regulator-grade capital markets operating system** designed to serve the complete lifecycle of financial operations for brokers, merchant bankers, listed issuers, and regulators. The platform integrates Order Management Systems (OMS), Execution Management Systems (EMS), Portfolio Management Systems (PMS), Corporate Actions processing, and comprehensive market data infrastructure into a unified, scalable, and compliant solution.

**Nepal is the first instantiation — not the architectural boundary.** Every piece of jurisdiction-specific logic (exchange protocols, regulatory rules, calendar systems, tax tables) is externalized into versioned, signed **Content Packs** (T1 Config / T2 Rules / T3 Executable), making it possible to on-board a new jurisdiction by publishing a new pack set — zero core-code changes required.

### 1.2 Strategic Objectives

1. **Unified Platform**: Consolidate fragmented capital markets operations into a single, coherent system
2. **Jurisdiction-Neutral Core**: Generic core with all country-specific logic externalized to signed content/plugin packs
3. **Regulatory Compliance**: Built-in compliance framework supporting SEBON, NRB (Nepal primary), SEBI, RBI (India), and international regulations (MiFID II, Dodd-Frank)
4. **Scalability**: Support from small brokers to large institutional operations with horizontal auto-scaling
5. **Real-time Operations**: Sub-millisecond order processing and market data distribution
6. **AI-Driven Intelligence**: Embedded AI for risk assessment, compliance monitoring, anomaly detection, and decision support — with full governance, explainability, and drift detection
7. **Multi-Asset Support**: Equities, derivatives, commodities, currencies, fixed income, and future-ready for digital assets and tokenized securities
8. **Cloud-Native**: Kubernetes-based deployment with multi-cloud abstraction (AWS/Azure/GCP/on-premise)
9. **Dual-Calendar Native**: Bikram Sambat and Gregorian calendars at the data layer — every timestamp, every event, every audit record
10. **Content/Plugin Extensibility**: All domain behaviour extensible via the T1/T2/T3 plugin taxonomy without modifying platform core
...
#### Corporate Actions & Issuer Services
...
| **Databases** | PostgreSQL, TimescaleDB, Redis, Elasticsearch, S3/MinIO, pgvector |
...
- **Workflow Engine**: Ghatana workflow runtime and saga orchestration
...
class OrderOrchestrationService {
  async placeOrder(orderRequest: OrderRequest): Promise<OrderResponse> {
    ...
    try {
      ...
      const validationResult = await this.orderValidationService
        .validate(orderRequest, correlationId);
      ...
      const riskCheck = await this.riskService
        .checkLimits(orderRequest, correlationId);
      ...
      await this.marginService
        .reserveMargin(orderRequest, correlationId);
      
      // Step 4: Submit to exchange
      const exchangeResponse = await this.executionService
        .submitOrder(orderRequest, correlationId);
      
      // Step 5: Update positions
      await this.positionService
        .updatePendingPosition(orderRequest, correlationId);
...
class Order {
  ...
  submit(): void {
    if (this.status !== OrderStatus.PENDING_VALIDATION) {
      throw new InvalidOrderStateError('Order must be validated before submission');
    }
  ...
  fill(fillQuantity: Quantity, fillPrice: Price): void {
    if (this.status !== OrderStatus.SUBMITTED && this.status !== OrderStatus.PARTIALLY_FILLED) {
    ...
    this.apply(new OrderFilledEvent({
      orderId: this.id,
      fillQuantity,
      fillPrice,
      remainingQuantity,
  ...
  cancel(): void {
    if (this.status === OrderStatus.FILLED || this.status === OrderStatus.CANCELLED) {
      throw new InvalidOrderStateError('Cannot cancel filled or already cancelled order');
    }
...
class DividendProcessingOrchestrator {
  async processDividend(dividendId: string): Promise<void> {
    ...
    const entitlements = await this.entitlementService
      .calculateEntitlements(dividend);
    ...
    {"name": "userId", "type": "string"},
    {
      ...
      "type": {
        ...
        "fields": [
          ...
          {"name": "orderType", "type": {"type": "enum", "name": "OrderType", "symbols": ["MARKET", "LIMIT", "STOP", "STOP_LIMIT"]}},
          {"name": "timeInForce", "type": {"type": "enum", "name": "TimeInForce", "symbols": ["DAY", "GTC", "IOC", "FOK"]}},
          {"name": "exchangeOrderId", "type": ["null", "string"], "default": null}
        ]
      }
    }
  ]
...
class OrderPlacedEventV1ToV2Upcaster implements EventUpcaster {
  canUpcast(event: DomainEvent): boolean {
    return event.eventType === 'ORDER_PLACED' && 
...
class IdempotentEventHandler {
  ...
  async handle(event: DomainEvent): Promise<void> {
    ...
    const isProcessed = await this.processedEventsStore
      .isProcessed(event.eventId);
