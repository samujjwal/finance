...
class AuditService {
  ...
  async log(entry: Omit<AuditLog, 'auditId' | 'timestamp' | 'timestampBs'>): Promise<void> {
    ...
    // Also send to Elasticsearch for search/analytics
...
10. **Observability Architecture**: Comprehensive observability with metrics collection (Prometheus), distributed tracing (Jaeger, OpenTelemetry), centralized logging (ELK stack), Grafana dashboards, and alerting (Alertmanager, PagerDuty).
