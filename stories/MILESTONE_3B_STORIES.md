...
UI for K-02 configuration engine: browse config tree (namespace/service/key hierarchy), edit values with type-aware inputs (string, number, boolean, JSON, duration), view history/diff, trigger override promotions. Maker-checker: edits submitted for approval inline with diff view. Bulk import from YAML. Config search with metadata filtering.
...
**Tests**: edit_string · edit_number · edit_json · duration_validation · maker_checker_diff · bulk_import · history_diff · search_filter · perf_tree_render_under_1sec
...
# EPIC W-01: WORKFLOW ENGINE (16 Stories)
...
Workflow engine core: define workflows in YAML/JSON DSL. Workflow: id, name, version, trigger (EVENT/SCHEDULE/MANUAL/API), steps[], error_handling, timeout. Step types: TASK (execute microservice call), DECISION (conditional branch), PARALLEL (concurrent steps), WAIT (delay or event), SUB_WORKFLOW. Stored in PostgreSQL with full version history. WorkflowDefined event.
...
**Tests**: yaml_parse · json_parse · step_task · step_decision · step_parallel · step_wait · version_history · event_emission · perf_parse_under_100ms
...
Client-facing document upload: per required document, upload button with file type validation (PDF/JPG/PNG, max 5MB). Drag-and-drop. Upload progress. Secure storage in S3/MinIO with client-specific prefix. Document status: PENDING_UPLOAD → UPLOADED → UNDER_REVIEW → APPROVED/REJECTED. Portal shows completion percentage. Email notification on document received.
