...
Implement `AuditSDK.log(action, resourceType, resourceId, { oldValue?, newValue?, metadata? })`. Auto-enriches with actor_id (from JWT context), correlation_id, timestamp_utc, timestamp_bs. Async write with in-memory buffer (max 1000) and retry on failure.
...
Scheduled job (nightly) that enforces retention policies: archive expired partitions to S3/MinIO, optionally drop archived partitions after verification. Log all retention actions to audit trail (meta-audit).
...
**Dependencies**: STORY-K07-011, S3/MinIO
...
Expose conversion functions via REST API: GET `/calendar/convert?from=greg&date=2024-04-13` and GET `/calendar/convert?from=bs&date=2081-01-01`. Return both representations plus DualDate object. Cache frequently requested dates.
...
## Feature K15-F02 — DualDate Generation & Storage Enforcement (2 Stories)

---
### STORY-K15-004: Implement DualDate type and generation utility
**Feature**: K15-F02 · **Points**: 2 · **Sprint**: 1 · **Team**: Alpha

Create `DualDate` type: `{ gregorian: { year, month, day, iso: string }, bs: { year, month, day, formatted: string }, fiscalYear: string, dayOfWeek: number }`. Utility `toDualDate(date)` generates complete DualDate from any input date.

**ACs**:
1. Given Gregorian Date, When toDualDate() called, Then complete DualDate with both calendars returned
2. Given DualDate, When serialized to JSON, Then both date representations included
3. Given fiscal year crossing (Shrawan 1), When generated, Then correct fiscal year label

**Tests**: dualDate_fromGreg · dualDate_fromBs · dualDate_fiscalYear · dualDate_jsonSerialization · dualDate_dayOfWeek

**Dependencies**: STORY-K15-001

---
### STORY-K15-005: Implement storage enforcement middleware
**Feature**: K15-F02 · **Points**: 2 · **Sprint**: 1 · **Team**: Alpha

Create database middleware that enforces DualDate storage: any table with a date/timestamp column must have corresponding _bs column. Validate on INSERT that both are populated and consistent. Reject single-calendar storage.

**ACs**:
1. Given INSERT with only utc timestamp, When _bs column empty, Then rejected with MISSING_BS_DATE
2. Given INSERT with both utc and bs, When dates are inconsistent, Then rejected with DATE_MISMATCH
3. Given enforced table, When schema migration adds date column, Then _bs column also required

**Tests**: enforce_missingBs_rejects · enforce_mismatch_rejects · enforce_bothPresent_accepts · enforce_migration_requiresBs
...
Implement `settlementDate(tradeDate, tPlusDays, jurisdiction): DualDate`. Counts only business days (skips weekends and holidays). T+1 from Friday in Nepal = Sunday (Saturday is weekend). Returns DualDate.
