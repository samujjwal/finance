...
**Project**: Siddhanta - Regulator-Grade Capital Markets Operating System
...
- **Content Pack Taxonomy (T1/T2/T3)**: All jurisdiction-specific behavior is externalized — T1 (Config/data-only), T2 (Rules/OPA-Rego), T3 (Executable/signed plugins). Siddhanta is a jurisdiction-neutral operating system.
- **Dual-Calendar (K-15)**: All timestamps carry both Gregorian and Bikram Sambat representations. The K-05 Event Bus standard envelope includes `timestamp_bs` and `timestamp_gregorian`.
...
**Purpose**: Shows the Siddhanta platform in the context of its users and external systems

**Key Contents**:
- System boundary and scope
- Primary actors (retail investors, brokers, regulators, etc.)
- External systems (exchanges, depositories, banks, KYC providers)
...
**Purpose**: Shows the high-level technology architecture and container-level components

**Key Contents**:
- Client-facing layer (web portal, mobile app, operator portal)
- API & integration layer (API gateway, FIX gateway, integration hub)
- Core services layer (OMS, PMS, CMS, RMS, identity, ledger, pricing, reconciliation, compliance, workflow)
- Data layer (PostgreSQL, TimescaleDB/pgvector, Redis, Elasticsearch, S3/MinIO)
- Event & messaging layer (Kafka, notification service)
- Observability layer (Prometheus, ELK, Jaeger)
